<x-guest-layout>
    <div class="min-h-screen flex items-center justify-center bg-primary/20 py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-4xl w-full bg-white/90 rounded-3xl shadow-2xl border border-primary/20 flex flex-col md:flex-row overflow-hidden">
            ...
            <div class="hidden md:flex flex-col justify-center items-center w-1/2 relative overflow-hidden p-0">
                ...
                <div class="flex flex-col items-center justify-center h-full w-full z-10 px-10 py-16" style="backdrop-filter: blur(8px); background: rgba(255,255,255,0.25); border-radius: 2rem; box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.18);">
                    <div class="bg-primary p-5 rounded-full shadow-lg mb-6 animate-bounce-slow">
                    ...
                    <div class="font-extrabold text-primary text-2xl mb-2 text-center tracking-tight drop-shadow">Halaman Admin</div>
                    <ul class="text-primary text-base leading-relaxed text-left font-medium mb-2 space-y-4">
                        <li class="flex items-start gap-2">
                            <svg class="w-6 h-6 text-secondary flex-shrink-0 mt-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                    ...
                    <div class="mt-6 text-xs text-primary/70 text-center">© {{ date('Y') }} Hakordia Fun Night Run</div>
                </div>
            </div>
            <!-- Right: Login Form -->
            <div class="w-full md:w-1/2 flex flex-col justify-center p-8 md:p-12">
                <h2 class="text-2xl font-bold text-primary mb-6 text-center md:text-left">Login Admin</h2>
    <!-- Session Status -->
    <x-auth-session-status class="mb-4" :status="session('status')" />
        ...
        <div>
            <x-input-label for="email" :value="__('Email')" />
                        <x-text-input id="email" class="block mt-1 w-full rounded-lg border-primary/40 focus:border-primary focus:ring-primary shadow-sm" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" />
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>
        <!-- Password -->
                    <div>
            <x-input-label for="password" :value="__('Password')" />
                        <x-text-input id="password" class="block mt-1 w-full rounded-lg border-primary/40 focus:border-primary focus:ring-primary shadow-sm" type="password" name="password" required autocomplete="current-password" />
            <x-input-error :messages="$errors->get('password')" class="mt-2" />
        </div>
        <!-- Remember Me -->
                    <div class="flex items-center justify-between">
            <label for="remember_me" class="inline-flex items-center">
                            <input id="remember_me" type="checkbox" class="rounded border-primary/40 text-secondary shadow-sm focus:ring-primary" name="remember">
            ...
            @endif
                    ...
                    <div>
                        <button type="submit" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-full shadow-lg text-lg font-bold text-white bg-primary hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition">
